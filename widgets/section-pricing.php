...
class Section_Pricing extends Widget_Base {
	...
	public function get_title() {
		return 'WHMCS Pricing Table';   // title to show on elementor
	}
...
//start of a control box
		...
		$this->add_control(
			'currency_number',
			[
				'label' => __( 'Currency Number', 'void_ewhmcse' ),
				'description' => __( 'The currency number from your WHMCS. Generally the default currency is numbered - 1', 'void_ewhmcse' ),
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default' => '1',
			]
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}}  .void-pricing-3.pt-type6 .pt-footer a' => '{{VALUE}};',
				],
			]
		);		
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}}  .void-pricing-3.pt-type6 .pt-footer a:hover' => '{{VALUE}};',
				],
			]
		);		
...
<?php if($settings['table_style'] == 'void-price-3' ) : ?>
	...
	<div class="void-pricing-3 pt-type6">
		<div class="pt-header">
			<h3 class="plan-title">
				<?php if( $settings['auto_title'] && $settings['whmcs_url']!='' && $settings['packedge_id'] && !\Elementor\Plugin::$instance->editor->is_edit_mode() ) { echo'<script language="javascript" src="'.$settings['whmcs_url'].'/feeds/productsinfo.php?pid='. $settings['packedge_id'] .'&get=name"></script>'; } else{echo $settings['title']; } ?>
			</h3>
			<h4 class="plan-price">
				<?php if($settings['oldprice']!=''){echo '<span class="old-price-span">'.$settings['oldprice'].'</span>'; } ?>
		    	<?php if( $settings['auto_price'] && $settings['whmcs_url']!='' && $settings['packedge_id'] && !\Elementor\Plugin::$instance->editor->is_edit_mode() ) { echo'<script language="javascript" src="'.$settings['whmcs_url'].'/feeds/productsinfo.php?pid='. $settings['packedge_id'] .'&get=price&billingcycle='. $settings['billingcycle'] .'&currency='.$settings['currency_number'].'"></script>'. $settings['billingafter']; } else{echo $settings['price']; } ?>
		    </h4>
		</div>
	...
	<div class="<?php echo $settings['table_style'].' '.$settings['pricing_style'];  ?>">
		<div class="pricing-box">
		    <div class="pricing-box-head">
		        ...
		        <span class="pbh-title"><?php if( $settings['auto_title'] && $settings['whmcs_url']!='' && $settings['packedge_id'] && !\Elementor\Plugin::$instance->editor->is_edit_mode() ) { echo'<script language="javascript" src="'.$settings['whmcs_url'].'/feeds/productsinfo.php?pid='. $settings['packedge_id'] .'&get=name"></script>'; } else{echo $settings['title']; } ?></span>
				<?php if( $settings['make_featured'] ): ?>
				<div class="sale-box">
					<span class="on_sale title_shop"><?php echo $settings['featured_title']; ?></span>
				</div>
